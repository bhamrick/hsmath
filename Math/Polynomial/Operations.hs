...
_naiveMultStep :: Num a => ([a] -> [a] -> [a]) -> [a] -> [a] -> [a]
_naiveMultStep _ _ [] = []
_naiveMultStep _ [] _ = []
_naiveMultStep _ [a] [b] = [a*b] -- Extra case to avoid extraneous 0
_naiveMultStep mult p@(a:as) q@(b:bs) = a*b : map (*a) bs `_addP` map (*b) as `_addP` (0: mult as bs)

_naiveMultP :: Num a => [a] -> [a] -> [a]
...
_karatsubaStep :: Num a => ([a] -> [a] -> [a]) -> [a] -> [a] -> [a]
_karatsubaStep _ _ [] = []
_karatsubaStep _ [] _ = []
...
_karatsubaStep mult as bs =
    ...
    in x `_addP` _shift 1 (z `_subP` (x `_addP` y)) `_addP` _shift 2 y

_karatsubaP :: Num a => [a] -> [a] -> [a]
_karatsubaP = _karatsubaStep _karatsubaP

_hybridMultStep :: Num a => ([a] -> [a] -> [a]) -> [a] -> [a] -> [a]
